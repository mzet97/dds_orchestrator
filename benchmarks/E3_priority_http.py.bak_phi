...
Metodologia:
  - Carga sustentada: 10 req/s com prioridade NORMAL (HTTP direto ao agente/orquestrador)
  - A cada inject_interval segundos: injeção de 1 requisição HIGH
  - Prioridade implementada via heapq em nível de aplicação Python
  - max_tokens=5: minimiza contribuição da inferência, isola latência de transporte
  - Duração: 5 minutos, 30 injeções HIGH

Usage:
    python E3_priority_http.py --url http://localhost:8080 --carga 10 --n 30 --duracao 300
"""
...
class PriorityQueueHTTP:
    """Fila de prioridade com heapq (aplicação Python)."""
    ...
    def enqueue(self, item: Dict, priority: int):
        """Adiciona item. Maior prioridade = processado primeiro."""
...
class PriorityBenchmarkHTTP:
    """Benchmark de priorização HTTP com heapq."""
    ...
    def __init__(self, base_url: str, carga_req_s: int = 10):
        ...
        self.queue = PriorityQueueHTTP()

    async def _send_request(self, session: aiohttp.ClientSession, priority: int) -> Dict:
        """
        Envia requisição via HTTP. Prioridade implementada em nível de aplicação
        (heapq no orquestrador Python), não em nível de transporte.
    ...
    async def inject_priority_message(self, session: aiohttp.ClientSession) -> Dict:
        ...
        return await self._send_request(session, priority=10)


async def run_benchmark(args):
    """Executa benchmark de priorização HTTP + heapq."""

    benchmark = PriorityBenchmarkHTTP(base_url=args.url, carga_req_s=args.carga)
